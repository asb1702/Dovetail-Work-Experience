...
class VotePicker extends Component {
  ...
  render() {
    return (
      <span>
        <Button className = { `bold` + (this.state.vote === 'up' ? ` selected` : ``) } variant = "success" onClick={ () => { this.incrementVote() }}>
          ↑
        </Button>         
        <h6>{this.state.votes}</h6>
        <Button className = { `bold` + (this.state.vote === 'down' ? ` selected` : ``) } variant = "danger" onClick={ () => { this.decrementVote() }}>
          ↓
        </Button>
      </span>
    );
